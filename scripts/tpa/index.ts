...
export async function TpaForm(player: Player) {
    ...
    form.show(player).then((result) => {
        if (!result.canceled) {
            if (result.selection == 0) TpaSearch(player);
            if (result.selection == 1) TpaPending(player);
            if (result.selection == 2) TPAPublicSettings(player, mecha.canTpa);
        }
    });
}

export async function TpaSearch(player: Player | any) {
    try {
        let form = new ActionFormData();
        ...
        for (let i = 0; i < players.mecha.length; i++) {
            ...
            if (mechaPlayer.canTpa) {
                ...
                form.button(
                    `${isThisPlayer ? "§lKAMU" : targetPlayer.nameTag}\n§6${playerLocation}`,
                    isThisPlayer
        ...
        form.show(player).then(async (result) => {
            ...
            if (resp.status) {
                player.sendMessage(
                    `§l§6[TPA] §r§7Kamu telah mengirim permintaan TP ke §2${targetPlayer.name}§7, mohon tunggu untuk disetujui.`
                );
...
export async function TpaPending(player: Player | any) {
    try {
        let form = new ActionFormData();
        form.title(`§l§cPlayer §2Teleport`);
        form.body(`§6Pilih player untuk target teleportasi.`);
        ...
        form.show(player).then((result) => {
            ...
            if (tpaRequest.fromName == player.name) {
                ...
                if (!targetPlayer) {
                    player.sendMessage(
                        `§r§l§e[§bTPA§e]§r §6Tidak bisa membuka permintaan. Pemain §a${tpaRequest.targetName} §6sedang nonaktif.`
                    );
                    return;
                }
                ...
                if (!fromPlayer) {
                    player.sendMessage(
                        `§r§l§e[§bTPA§e]§r §6Tidak bisa membuka permintaan. Pemain §a${tpaRequest.fromName} §6sedang nonaktif.`
                    );
                    return;
                }
            }
...
export function TPAPublicSettings(player: Player, currentSetting: boolean) {
    ...
    form.show(player).then(async (result) => {
        if (!result.canceled) {
            ...
            const resp = await MechAPI.changePublicTpa(player, canTpa as boolean);
            if (resp.status) {
...
export async function TpaUI(fromPlayer: Player, player: Player) {
    ...
    new MessageFormData()
        ...
        .body(
            `§r§lKamu mendapat permintaan teleport dari §6${fromPlayer.nameTag}§r§l untuk pergi ke lokasimu, apakah kamu ingin menerimanya?`
        )
        ...
        .then((result) => {
            if (result.canceled && result.cancelationReason == FormCancelationReason.UserBusy) {
                // Sibuk
                player.sendMessage(
                    `§l§6[TPA] §r§7Kamu mendapat permintaan teleport dari ${fromPlayer.nameTag}§7, silahkan cek menu TPA Tertunda untuk menerima atau menolak.`
                );
                return;
            }
