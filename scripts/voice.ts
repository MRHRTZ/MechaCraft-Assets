...
export function VoicePageUser(player: Player) {
    const VoicePage = new ActionFormData()
...
function clientSettings(player: Player) {
    var ServerKey = "";
    ...
    new ModalFormData()
        ...
        .textField("Kunci client", "abc123", ServerKey as string)
        .show(player)
        .then((results) => {
...
function internalSettings(player: Player) {
    new ModalFormData()
...
function externalSettings(player: Player) {
    Network.ShowSettings(player);
}

function autoConnectSettings(player: Player) {
    ...
    new ModalFormData()
        ...
        .textField("Alamat IP", "127.0.0.1", IP as string)
        .textField("Port", "9051", Port ? Port.toString() : "")
        .textField("Kunci Server", "abc123", ServerKey as string)
        .show(player)
        .then((results) => {
            if (results.canceled) return;
...
function connectServer(params: { IP: string; PORT: number; Key: string; source: Player }) {
    Network.Connect(params.IP, params.PORT, params.Key, params.source);
}

function connectClient(player: Player) {
    var ServerKey = "";
...
function autoConnect(player: Player) {
    const IP = world.getDynamicProperty("autoConnectIP") as string;
